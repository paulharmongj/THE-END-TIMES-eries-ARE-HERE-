...
arma.1 <- gls(Days~year,correlation=corARMA(p = 17, q = 0),method="ML", data = bts)
summary(arma.1)
phi.1<-arma.1$modelStruct$corStruct
phi.1<-as.numeric(as.character(phi.1))
phi.1<-c(1,-phi.1)
root.phi.1<-polyroot(phi.1)
a<-a1<-as.matrix(cbind(phi.1[2:18],as.numeric(abs(root.phi.1)>1)))
sum(subset(a,a[,2]==1)[,1])
#the sum of the phi's with roots>1 are greater than 1 so the
...
<<tidy.opts=list(width.cutoff=50)>>=
#Fit an AR 18
arma.18 <- gls(Days~year,correlation=corARMA(p = 18, q = 0),method="ML", data = bts)
summary(arma.18)
...
@
\item %8
We can fit the MA(1) model on the differenced series. 
<<tidy.opts = list(width.cutoff = 50)>>=
...
\item %9
The test using the confidence intervals tells us that this model is ____. It addresses the researcher's 
