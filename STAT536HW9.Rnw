...
\item %5
Examining the confidence intervals for all of the lower order auto-regressive coefficients will tell us if they are different from zero at the 5\% significance. We can see that very few of the intervals do not contain zero; only the intervals for the second and sixth lags in fact are  different from zero.\\
